...
float ZenDecibelParameter::getValue() const
{
	...
	{		
		////DBG("In ZenDecibelParameter::getValue() of " << this->paramID << " returning smoothed value: " << getNextSmoothedValue());	
		return const_cast<ZenDecibelParameter*>(this)->getNextSmoothedValue();
	}
	return value.load();
}

float ZenDecibelParameter::getValueUnsmoothed() const
{
	return value.load();
}
...
{
	//double updateVal = state.getProperty(owner.valuePropertyID, defaultValue);
	DBG("In updateFromValueTree of " << this->paramID << " setting from state to: " << state.getProperty(owner.valuePropertyID, defaultValue).toString());
...
void ZenDecibelParameter::copyValueToValueTree()
{
	...
	if (state.isValid())
		state.setProperty(owner.valuePropertyID, getValueUnsmoothed(), owner.undoManager);
}
