...
{
	if (m.isNoteOn())           return "Note on: " + String(m.getNoteNumber()) + " (" + 
		MidiMessage::getMidiNoteName(m.getNoteNumber(), true, true, 3) + ") Vel: " + String(m.getVelocity());
	if (m.isNoteOff())          return "Note off: " + String(m.getNoteNumber()) + " (" + 
		MidiMessage::getMidiNoteName(m.getNoteNumber(), true, true, 3) + ")";
	...
	if (m.isController())
	{
		...
		String name = "Controller [" + String(m.getControllerNumber()) + "]";
		if (!temp.isEmpty())
			name += " " + temp;
...
void ZenMidiVisualiserComponent::handleNoteOff(MidiKeyboardState*, int midiChannel, int midiNoteNumber, float velocity)
{
