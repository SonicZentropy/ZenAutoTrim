...
public:
	...
	ZenRotaryFilmStripSlider(const String& imgName, const TextEntryBoxPosition inPos = TextBoxCentered, 
		const int inTextBoxWidth = 60, const int inTextBoxHeight = 24,
		const int numFrames= 63, const int knobWidth = 64, const int knobHeight = 64,
		const bool stripIsHorizontal = false);
	...
	const Image& getFilmStrip() const { return filmStrip; }
	...
	void setSkewFactorFromMidPoint(const double midValue);

	float getMidPointValue() { return midPoint; }
