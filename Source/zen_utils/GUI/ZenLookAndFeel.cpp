...
#include "zen_utils/components/ZenLabelDisplay.h"
...
void ZenLookAndFeel::drawLabel(Graphics& g, Label& label)
{
	...
	if (!label.isBeingEdited())
	{
		...
		{
			int textPrecision = zenLabel->getTextDecimalPrecision();
			
			DBG("Parent name: " << zenLabel->getParentComponent()->getName());

			ZenRotaryFilmStripSlider* zenSlider = dynamic_cast<ZenRotaryFilmStripSlider*>(zenLabel->getParentComponent());
			if (nullptr != zenSlider) // Don't draw shadow on basic sliders
			{
				DBG("Parent is zen rotary");
	...
	//g.drawRect(label.getLocalBounds().withSizeKeepingCentre(label.getWidth()/*-10*/, label.getHeight()));
}

Font ZenLookAndFeel::getLabelFont(Label& label)
{
	ZenRotaryEditorLabel* reLabel = dynamic_cast<ZenRotaryEditorLabel*>(&label);
	if (nullptr != reLabel)
	{
	//	DBG("Label is rotary");
		ZenRotaryFilmStripSlider* textBoxSlider = dynamic_cast<ZenRotaryFilmStripSlider*>(reLabel->getParentComponent());
		Slider::TextEntryBoxPosition textpos = textBoxSlider->getTextBoxPosition();
		if (textpos == Slider::TextBoxCentered)
		{
...
// 	{
		if (zenSlider.hasValidFilmStrip())
		{
			...
			//float skewValue = zenSlider.getSkewFactor();
			double cvalue = zenSlider.getValue();
			//double minimum = zenSlider.getMinimum();
			//double maximum = zenSlider.getMaximum();
			int numFrames = zenSlider.getNumFrames();
			//double midValue = zenSlider.getMidPointValue();
			double valToProp = zenSlider.valueToProportionOfLength(cvalue);
			//double propToValue = zenSlider.proportionOfLengthToValue(0.5f);
			//int value = (cvalue - minimum) / (maximum - minimum) * (numFrames - 1);
			//float value = value = (cvalue - minimum) / (maximum - minimum);
			
			//double range = maximum - minimum;
			//double origSkew = ((range / 2) - minimum) / range;
			//int skewedValue = getClamped(std::floor((value / origSkew) * (numFrames - 1)), 0, (numFrames-2));

			int skewedValue = (numFrames-1) * valToProp;
			
			if (zenSlider.getFilmstripIsHorizontal())
			{
				g.drawImage(zenSlider.getFilmStrip(), sliderRect.getX(), sliderRect.getY(), zenSlider.getKnobWidth(), zenSlider.getKnobHeight(), skewedValue * zenSlider.getFrameWidth(), 0, zenSlider.getFrameWidth(), zenSlider.getFrameHeight());
			}
			else
			{
				g.drawImage(zenSlider.getFilmStrip(), sliderRect.getX(), sliderRect.getY(), zenSlider.getKnobWidth(), zenSlider.getKnobHeight(), 0, skewedValue * zenSlider.getFrameHeight(), zenSlider.getFrameWidth(), zenSlider.getFrameHeight());
			}
		}
	//}
...
void ZenLookAndFeel::drawZenComboBox(Graphics& g, int width, int height, bool isButtonDown, int buttonX, int buttonY, int buttonW, int buttonH, ZenComboBox& inBox)
{
	...
	const Image& center = inBox.getCenterImage();
	const Image& right = inBox.getRightImage();
	g.setOpacity(1.0f);
	g.drawImage(left, 0, 0, left.getWidth(), inBox.getHeight(),
		0, 0, left.getWidth(), left.getHeight());
	g.drawImage(center, left.getWidth(), 0, inBox.getWidth() - left.getWidth() - right.getWidth(), inBox.getHeight(),
		0, 0, center.getWidth(), center.getHeight());
	g.drawImage(right, inBox.getWidth() - right.getWidth(), 0, right.getWidth(), inBox.getHeight(),
		0, 0, right.getWidth(), right.getHeight());
	...
	if(! inBox.isEnabled())
	{
		...
		g.setOpacity(0.6f);		
		g.fillRoundedRectangle(0, 0, inBox.getWidth(), inBox.getHeight(), 5);		
	}
}
...
void ZenLookAndFeel::initColours()
{
