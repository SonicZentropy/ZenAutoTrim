...
public:
	...
	static Type decibelsToDBGain(const Type decibels, const Type minusInfinityDb = (Type)defaultMinusInfinitydB)
	{		
		return (decibels > minusInfinityDb) ? std::pow ((Type) 10.0, decibels * (Type) 0.05) : Type();
	}
	
	/// <summary>Converts an RMS value to its equivalent Decibel value.</summary>
	/// <param name="inRMS">Input value in RMS to be converted to decibels.</param>
	/// <param name="decibelRMSCalibrationValue">Additional db to add to result for calibration (3 default)</param>
	/// <returns>Raw decibel FS value for the input RMS</returns>
	static float rmsToCalibratedDecibels(const float& inRMS, const float& decibelRMSCalibrationValue = 3.0f)
	{
		return (Decibels::gainToDecibels(inRMS) + decibelRMSCalibrationValue);
	}
	...
	static double mapProperNormalizedValueToRawDecibelGain(double normGainValue, float minimumDecibels, float maximumDecibels, float mappedMidpointOfRange)
	{
		...
		return decibelsToDBGain(valueInDecibels, (double)minimumDecibels);
	}
